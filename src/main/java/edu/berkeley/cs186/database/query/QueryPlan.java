...
public class QueryPlan {
  ...
  private QueryOperator pushDownSelects(QueryOperator source, int except) throws QueryPlanException, DatabaseException {
    ...
    for (int i = 0; i < this.selectColumnNames.size(); i++) {
      ...
      String selColumn = this.selectColumnNames.get(i);
      PredicateOperator operator = this.selectOperators.get(i);
      DataBox value = this.selectDataBoxes.get(i);

      try {
        source = new SelectOperator(source, selColumn, operator, value);
  ...
  private QueryOperator minCostSingleAccess(String table) throws DatabaseException, QueryPlanException {
    ...
    int minSelectIdx = -1;
    for(int selIndx : selectIndices) {
      String indexCol = this.selectColumnNames.get(selIndx);
      PredicateOperator op = this.selectOperators.get(selIndx);
      DataBox val = this.selectDataBoxes.get(selIndx);

      QueryOperator newOp = new IndexScanOperator(this.transaction, table, indexCol, op, val);
      int cost = newOp.getIOCost();
      if(cost < minCost) {
        minOp = newOp;
        minCost = cost;
        minSelectIdx = selIndx;
      }
    }
